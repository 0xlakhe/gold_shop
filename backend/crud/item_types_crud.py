...
def createNewType(db: Session, itemName: ItemTypeCreate):
...
def deleteType(db: Session, type_id: int):
    db_type = db.query(ItemType).filter(ItemType.id == type_id).first()
...
def updateType(db:Session,type_id:int, item_name:ItemTypeCreate):
    ...
    name_taken= db.query(ItemType).filter(ItemType.name==item_name.name, ItemType.id!=type_id).first()
